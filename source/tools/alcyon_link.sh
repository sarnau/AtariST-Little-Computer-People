...
#    The spec describes the SHIPPED build, so it only applies when the
#    objects were compiled with no gating defines; a test build has
#    different code and different relocation sites, and remapping it
#    would (rightly) fail on the first site the spec has never seen.
cp LCP.PRG LCP_nobss.PRG        # pre-remap copy, for bss_remap.py --gen
if [ -s CPPFLAGS ]; then
    echo "bss_remap: SKIPPED -- built with $(cat CPPFLAGS); the layout"
    echo "           spec describes the shipped build only, and this"
    echo "           binary is NOT byte-identical by construction."
else
