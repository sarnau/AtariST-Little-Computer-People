...
targets sit in BSS, because the two linkers packed the same commons
at different offsets.

The ROM's allocation is therefore carried as a checked-in layout
spec, tools/rom_bss_layout.tsv: one row per referenced BSS cell,
keyed by PORT SYMBOL + OFFSET (stable across relinks) with the ROM
address it must land on.  The normal link-time invocation
...
the header BSS size from the spec.  The original binary is NOT read.

Regenerate the spec (only needed after a layout-affecting source
change) with the reference binary present:

    python3 tools/bss_remap.py --gen LCP.PRG [orig.prg]

--gen pairs the two byte-identical relocation streams site-by-site
and verifies the port->ROM translation is a consistent one-to-one
mapping before writing the table; that consistency check is the
proof that the port's reference structure (which symbol each site
addresses, at which offset) matches the ROM's exactly.
...
def gen():
    ...
    with open(TSV, 'w') as f:
        f.write('# rom_bss_layout.tsv -- the original linker\'s BSS '
                'allocation, keyed by port symbol+offset.\n'
                '# Generated by bss_remap.py --gen against '
                'DATA/LCP_ORG.PRG; regenerate after any\n'
                '# layout-affecting source change and review the '
                'diff.  Columns: symbol offset rom_addr.\n')
