 ...
 * and calls sprite_update_body().  Then:
 *   - asserts the compositor wrote non-zero pixels into lcp_sprite_img
 *   - asserts sprite_active_x[3] / sprite_active_y[3] track lcp_x/y
 *   - dumps lcp_sprite_img to sprite_slot3.pgm as a portable graymap
...
/* Dump lcp_sprite_img[168 shorts] as a 64x21 1-bit silhouette PGM.
...
char ** argv;
{
        ...
        memset(lcp_sprite_mask, 0, 168 * sizeof(short));
        ...
        nonzero = 0;
        for (i = 0; i < 168; i++)
                if (lcp_sprite_img[i]) { nonzero = 1; break; }
        CHECK(nonzero, "lcp_sprite_img is all zeros after compose");
        ...
        CHECK(sprite_active_x[3] == expected_x,
              "sprite_active_x[3] mismatch after compose");
        CHECK(sprite_active_y[3] == expected_y,
              "sprite_active_y[3] mismatch after compose");

        /* Dump the composited slot 3 image for visual inspection. */
        dump_pgm("sprite_slot3.pgm", lcp_sprite_img, 21, 4);
