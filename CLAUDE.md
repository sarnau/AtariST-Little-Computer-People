...
**The verify's expectations are GENERATED now** (fixed 2026-09-07).
...
hand-patch, so nothing to forget.

Two design decisions in that generator are worth knowing, because both
are about keeping the report free of noise a real drift could hide in:

  * **It expands the truncated linkage names**, against the identifiers
    the port declares at FILE SCOPE, and only where the expansion is
  ...
  * **It omits the ~98 library symbols.**  `___pname`, `__afreeb` and
    `_malloc_` have no declaration in this tree, so the repo cannot say
    what they should be called, and Ghidra's names for them follow no
    reproducible rule (`___pname` but `_afreebase`, `_iob`, `_ctype`).
...
Three things to get right when building the rename list:
  ...
  * **lcp_sym.68k carries 8-char TRUNCATED linkage names.**  Pushing
    ...
    `tools/gen_ghidra_verify.py` do it and read what verify reports.
